...
- 2026-06-21: **Holiday banner non-clickable + cleaner Upcoming tiles.** Home holiday banner converted from TouchableOpacity to a plain View (no navigation) — verified cursor:auto. Services: removed the amber tile background/border/accent (tileUpcoming) around upcoming tiles; tiles are plain white with dimmed content + only the pulsing yellow Upcoming badge as the highlight. (index.tsx, services.tsx)
- 2026-06-21: **Pulse animation on Upcoming badge.** Added a subtle looping pulse (scale 1→1.09 + opacity 1→0.72, 1.1s ease-in-out) via a new `UpcomingBadge` Animated component in services.tsx. Verified live: transform animates frame-to-frame (scale 1.005→1.065) on all 3 badges.
